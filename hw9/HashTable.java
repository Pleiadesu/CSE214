...
public class HashTable<K, V> implements Map<K, V> {
    ...
    public V get(K key) {
        ...
        if(buckets[index(key)] != null){
			for(Entry h: buckets[index(key)]){
				if(h.key().hashCode() == key.hashCode()){
					return (V)h.value();
				}
			}
		}
    ...
    public V put(K key, V value) {
        ...
        if(buckets[index(key)] != null){
			...
			buckets[index(key)] = new ArrayList<Entry<K, V>>();
		}
		//add new entry - TODO
		buckets[index(key)].add(0, new HashEntry(key, value));
		size++;
    ...
    protected void rehash() {
        ...
        ArrayList<Entry<K, V>>[] newBucket = (ArrayList<Entry<K, V>>[]) new ArrayList[capacity];
        ...
        for(int i = 0; i < newBucket.length; i++){
			if(newBucket[i] != null){
				for(Entry h: newBucket[i]){
					put((K)h.key(), (V)h.value());
				}
			}
